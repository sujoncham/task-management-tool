        ...
        <div className="container mx-auto px-10 mt-5 py-10">
            <h1 className="text-center text-3xl font-bold mb-5">To-Do Task - {tasks?.length}</h1>
                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-5 p-3 mb-5">
                    {
                        tasks?.map(task => <div className={`${task.role === 'completed' ? 'hidden': 'flex justify-between items-center shadow-3xl bg-base-200 py-4 rounded-md'}`} key={task._id} task={task}>
                            <div className={`${task.role === 'completed' ? 'hidden': 'flex justify-between items-center shadow-3xl bg-base-200 py-4 rounded-md'}`}>
                            <input onClick={() => handleCompleted(task._id)} disabled={task.role === 'completed'} type="checkbox" className="mx-2 cursor-pointer" /> 
